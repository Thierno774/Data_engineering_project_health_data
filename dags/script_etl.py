...
project_id = "capable-stream-449610-f1"
dataset_id = "staging_dataset"
transform_dataset_id = "transform_dataset"
reporting_dataset_id = "reporting_dataset"

source_table = f"{project_id}.{dataset_id}.global" # Main table loaded form CSV
# country-specific tables to be created 
...
with DAG (
...
) as dag: # Task1 :  check if the file exists in GCS 
    check_file_exists = GCSObjectExistenceSensor (
                task_id = "check_file_exists", 
                bucket = "bkt-src-global-datas", 
                object = "global_health_data.csv",
                timeout = 300, # Maximum wait time in seconds 
                poke_interval = 30, 
                mode = "poke", # Use "Poke mode for synchronous checking"
    )
    # Task2 : load csv from GCS to BigQuery 
    load_csv_to_bigquery = GCSToBigQueryOperator(
                    task_id = "load_csv_to_bq", 
                    bucket = "bkt-src-global-datas", # replace with your bicket name 
    ...
    for country in countries: 
        ...
        # Task ti create view for earcj country-specific table with selected columpns and filter 
